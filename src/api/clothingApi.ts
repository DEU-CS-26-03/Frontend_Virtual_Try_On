...
export const uploadGarment = async (file: File, category?: string) => {
  ...
  const res = await fetch(API_ROUTES.GARMENTS, { // 👈 API_ROUTES 사용
    method: "POST",
    body: formData,
  });
...
export const getGarments = async (params?: {
  ...
  const res = await fetch(`${API_ROUTES.GARMENTS}?${query.toString()}`, { // 👈 API_ROUTES 사용
    method: "GET",
...
export const getGarmentById = async (id: string) => {
  const res = await fetch(`${API_ROUTES.GARMENTS}/${id}`, { // 👈 API_ROUTES 사용
    method: "GET",
    headers: { Accept: "application/json" },
  });
