      ...
      <div className="max-w-[1600px] mx-auto px-10 py-20">
        <div className="mb-20">
          ...
          <h2 className="text-6xl font-[1000] tracking-tighter">
            안녕하세요. <span className="text-gray-300 italic">{user?.nickname || "USER"}</span>
          </h2>
          <p className="mt-4 text-gray-500 font-medium">{user?.email}</p>
        </div>

        <div>
          <h3 className="text-xl font-black mb-10 border-b border-gray-200 pb-4">마이 페이지</h3>
          
          {history.length === 0 ? (
            <div className="py-20 text-center border-2 border-dashed border-gray-200 rounded-sm">
              <p className="text-gray-400 font-bold">아직 피팅 내역이 없습니다. 원하는 의상을 골라 시착해보세요!</p>
            </div>
          ) : (
