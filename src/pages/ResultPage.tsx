...
// import { createTryOn, getTryOnStatus } from "../api/tryonApi"; // 실제 연동 시 주석 해제
// import { getResultById } from "../api/systemApi"; // 실제 연동 시 주석 해제

const ResultPage = () => {
  const location = useLocation();
  const navigate = useNavigate();
  
  // Fitting 페이지에서 넘어온 데이터들
  ...
  useEffect(() => {
    ...
    const simulateFitting = () => {
      ...
      // 3초 후 결과 출력 (실제 서버 통신 대신 더미 이미지 세팅)
      pollTimer.current = setTimeout(() => {
        setResultImage("https://picsum.photos/seed/fitting_res/800/1200"); // 결과 이미지 예시
        setLoading(false);
      }, 3000);
    };
      ...
      try {
        ...
        const checkStatus = async (id: string) => {
          ...
          if (data.status === "completed") {
            ...
            pollTimer.current = setTimeout(() => checkStatus(id), 2500);
          }
        };
        checkStatus(task.tryon_id);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 font-sans text-gray-900">
      ...
      <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-10 px-6 mb-16">
        ...
        {/* 1. 왼쪽: 원본 이미지 (Before) */}
        <div className="bg-white p-4 rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col">
          ...
          <div className="relative w-full h-[650px] bg-gray-100 rounded-[1.8rem] overflow-hidden">
            <img 
              src={preview} 
              className="w-full h-full object-cover" // 빈 공간 없이 꽉 채움
              alt="Original" 
            />
        ...
        {/* 2. 오른쪽: AI 결과 이미지 (After) */}
        <div className="bg-white p-4 rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col relative">
          ...
          <div className="relative w-full h-[650px] bg-gray-100 rounded-[1.8rem] overflow-hidden flex items-center justify-center">
            {loading ? (
              ...
              <>
                <img 
                  src={resultImage!} 
                  className="w-full h-full object-cover animate-fadeIn" // ✅ 왼쪽과 동일하게 꽉 채움
                  alt="Result" 
                />
                <div className="absolute top-4 left-4 bg-blue-600 text-white px-4 py-1 rounded-full text-xs font-bold shadow-lg">
                  AFTER
                </div>
                <button 
        ...
        <div className="max-w-4xl mx-auto px-6 animate-slideUp">
          <div className="bg-white py-12 px-8 rounded-[3rem] shadow-2xl border border-gray-100 text-center">
            ...
            <div className="flex justify-center gap-4 mb-10">
              {[1, 2, 3, 4, 5].map((idx) => (
                <button key={idx} onClick={() => { setRating(idx); setShowRec(true); }}>
                  <Star 
                    size={48} 
                    className={`transition-all duration-300 ${idx <= rating ? "fill-yellow-400 text-yellow-400 scale-110" : "text-gray-100 hover:text-gray-300"}`} 
                  />
                </button>
              ))}
            </div>
            ...
            <div className="flex justify-center gap-4">
              <button className="flex items-center gap-2 px-14 py-5 bg-black text-white rounded-2xl font-bold shadow-xl hover:scale-105 transition-all">
                <ThumbsUp size={20} /> 만족해요!
              </button>
              <button onClick={() => navigate("/")} className="flex items-center gap-2 px-14 py-5 bg-white border border-gray-200 rounded-2xl font-bold hover:bg-gray-50 transition-all">
                <ThumbsDown size={20} /> 아쉬워요..
              </button>
            </div>
          </div>
